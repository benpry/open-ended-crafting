...
feature_names: list[KV]
This is a list of key-value pairs mapping the values of features (e.g. a cook level of 1) to their descriptions (e.g. "cooked"). Each key-value pair should have a key and a value.
...
Your job is to reflect on the experiment results, decide what might have gone wrong, and propose a plan for how to tweak the game to improve the results.
...
class ReflectionResponse(BaseModel):
    reflection: str = Field(
        description="Text containing a verbalized reflection on the experiment results"
    )
    plan: str = Field(
        description="Text containing a plan for how to tweak the game to improve the results"
    )
...
class ExperimentOptimizer:
    ...
    def reflect(self, df_sims: pd.DataFrame):
        ...
        print(f"Iteration {self.iteration} metrics: {metrics}")
        self.history.append(
            types.Content(
                role="user",
                parts=[
        ...
        # def inspect_sample_round_tool(round_num: int):
        #     return inspect_sample_round(df_sims, round_num)
    ...
    def propose(self):
        ...
        print(f"Proposing with history length: {len(self.history)}")
    ...
    def run(self, initial_game_descriptor: GameDescriptor, max_iter: int = 5):
            ...
            types.Content(
                ...
                parts=[
                    types.Part.from_text(
                        text="Here is the initial game descriptor: "
                        + game_descriptor_to_llm(
        ...
        for i in range(max_iter):
            ...
            reflection = self.reflect(df_sims)
            print(f"Reflection: {reflection}")
            self.curr_game_descriptor = self.propose()
            print(f"New game descriptor: {self.curr_game_descriptor}")
            self.iteration += 1
...
if __name__ == "__main__":
    ...
    optimizer.run(potions_game_descriptor, max_iter=5)
